...
class EventTimeTriggerTest {
    ...
    @Test
    void firesWhenTimeReachesWindowEnd() {
        EventTimeTrigger<String> trigger = new EventTimeTrigger<>();
        TimeWindow window = new TimeWindow(0, 10);

        assertEquals(WindowAssigner.TriggerResult.CONTINUE, trigger.onEventTime(9, window));
        assertEquals(WindowAssigner.TriggerResult.FIRE_AND_PURGE, trigger.onEventTime(10, window));
        assertEquals(WindowAssigner.TriggerResult.FIRE_AND_PURGE, trigger.onEventTime(11, window));
        assertEquals("EventTimeTrigger", trigger.toString());
    }

    @Test
    void ignoresElementsAndProcessingTime() {
        EventTimeTrigger<String> trigger = new EventTimeTrigger<>();
        TimeWindow window = new TimeWindow(0, 10);
